...
class TestGravityRemoval:
    ...
    def apply_highpass_filter(
        self, data: np.ndarray, cutoff: float = 0.3, fs: float = 50.0
    ) -> np.ndarray:
    ...
    def test_gravity_removal_reduces_dc_offset(self):
        ...
        assert abs(np.mean(az_filtered)) < 1.0, f"Gravity not removed: mean={np.mean(az_filtered)}"
