...
def generate_figure_1():
    ...
    datasets = [
        {
            "name": "all_users_data_labeled.csv",
            "type": "training",
            "date": "2024-06",
            "size": 385326,
        },
        {
            "name": "2025-03-23-accelerometer.xlsx",
            "type": "raw_accel",
            "date": "2025-03",
            "size": 345418,
        },
        {
            "name": "2025-03-23-gyroscope.xlsx",
            "type": "raw_gyro",
            "date": "2025-03",
            "size": 345418,
        },
    ...
    ax.annotate(
        "", xy=(3.6, 2), xytext=(4.8, 1), arrowprops=dict(arrowstyle="->", color="red", lw=2)
    )
    ax.text(
        4.5,
        1.5,
    ...
    # Add legend
    legend_patches = [
        mpatches.Patch(color=c, label=l.replace("_", " ").title()) for l, c in colors.items()
    ]
    ...
    ax.set_title(
        "Figure 1: Dataset Timeline & Newest Fused File Selection Logic", fontweight="bold", pad=20
    )
    ax.set_yticks([])
    ax.set_xticks([])
...
def generate_figure_2():
    ...
    fig.suptitle("Figure 2: Sampling Rate & Time-Gap Quality Control", fontweight="bold", y=1.02)
    plt.tight_layout()
...
def generate_figure_3():
    ...
    axes[1].hist(
        az_before, bins=40, alpha=0.6, color="red", label="Before (with gravity)", density=True
    )
    axes[1].hist(
        az_after, bins=40, alpha=0.6, color="green", label="After (filtered)", density=True
    )
    ...
    fig.suptitle(
        "Figure 3: Gravity Removal Impact on Acceleration Distributions", fontweight="bold", y=1.02
    )
    plt.tight_layout()
...
def generate_figure_4():
    ...
    # Margin distribution
    axes[2].hist(
        margin, bins=40, color="mediumseagreen", edgecolor="black", alpha=0.7, density=True
    )
    ...
    fig.suptitle(
        "Figure 4: Proxy Metrics Distributions with Paper-Backed Thresholds",
        fontweight="bold",
        y=1.02,
    )
    plt.tight_layout()
...
def generate_figure_5():
    ...
    fig.suptitle(
        "Figure 5: Drift Metrics Overview (Production vs Training Baseline)",
        fontweight="bold",
        y=1.02,
    )
    plt.tight_layout()
...
def generate_figure_6():
    ...
    axes[0].set_title(
        "A) Drift Metric Trend Over Batches (Retraining Trigger View)", fontweight="bold"
    )
    ...
    fig.suptitle(
        "Figure 6: Drift Over Time - Monitoring for Retraining Triggers", fontweight="bold", y=1.02
    )
    plt.tight_layout()
...
def generate_figure_7():
    ...
    axes[0].axhline(
        0.5, color="red", linestyle="--", linewidth=1.5, label="Low observability threshold"
    )
    axes[0].legend(fontsize=8)
    ...
    fig.suptitle(
        "Figure 7: ABCD Cases - Dominant/Non-dominant Hand Detection Heuristics",
        fontweight="bold",
        y=1.02,
    )
    plt.tight_layout()
