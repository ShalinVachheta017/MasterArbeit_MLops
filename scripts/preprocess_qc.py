...
class PreprocessQC:
    ...
    def _add_check(
        self, name: str, passed: bool, severity: str, message: str, details: Dict = None
    ):
        """Add a check result."""
        self.checks.append(
            CheckResult(
    ...
    def check_missingness(self, df: pd.DataFrame, sensor_cols: List[str]) -> None:
        ...
        self._add_check(
            ...
            severity="HIGH",
            message=(
                f"Max missing: {max_missing_pct:.2f}%"
                if max_missing_pct >= 5.0
                else f"Max missing: {max_missing_pct:.2f}%"
            ),
            details={
    ...
    def check_sampling_rate(self, df: pd.DataFrame, expected_hz: float = 50.0) -> None:
        ...
        try:
            ...
            self._add_check(
                ...
                severity="HIGH",
                message=(
                    f"Actual: {actual_hz:.1f}Hz, Expected: {expected_hz}Hz"
                    if hz_diff >= tolerance
                    else f"OK ({actual_hz:.1f}Hz)"
                ),
                details={
                    "actual_hz": round(actual_hz, 2),
                    "expected_hz": expected_hz,
                    "deviation": round(hz_diff, 3),
                },
            )
            ...
            jitter_pct = (
                (std_diff_ms / expected_interval_ms) * 100 if expected_interval_ms > 0 else 0
            )
            ...
            self._add_check(
                ...
                severity="MEDIUM",
                message=(
                    f"Jitter: {jitter_pct:.2f}% of interval"
                    if jitter_pct >= 5.0
                    else f"OK (jitter: {jitter_pct:.2f}%)"
                ),
                details={
    ...
    def check_units(self, df: pd.DataFrame, data_type: str) -> None:
        ...
        if az_col:
            ...
            gravity_check = (detected_units == "m/s²" and -12 < az_mean < -7) or (
        ...
        self._add_check(
            ...
            details={
                ...
                "gravity_check": bool(gravity_check),
            },
        )
    ...
    def check_gyro_units(self, df: pd.DataFrame, data_type: str) -> None:
        ...
        self._add_check(
            ...
            severity="HIGH",
            message=(
                f"Detected: {detected_units} (max={max_abs:.1f})"
                if expected is not True
                else f"OK (deg/s, max={max_abs:.1f})"
            ),
            details={
                "detected_units": detected_units,
    ...
    def check_normalization(self, data: np.ndarray) -> None:
        ...
        self._add_check(
            ...
            severity="CRITICAL",
            message=(
                f"Mean per channel: {mean_per_channel.round(3).tolist()}" if not mean_ok else "OK"
            ),
            details={
...
def main():
    ...
    parser.add_argument("--input", type=str, required=True, help="Input file path (CSV or NPY)")
    parser.add_argument(
        "--type",
        type=str,
        default="production",
