...
class EnergyOODDetector:
    ...
    def compute_energy(
        ...
        energy = -T * (max_logits.squeeze() + np.log(
            np.sum(np.exp(logits_scaled - max_logits), axis=1)
        ))
    ...
    def calibrate(self, in_dist_energies: np.ndarray):
        ...
        self.config.energy_threshold_critical = self.in_dist_energy_mean + 3 * self.in_dist_energy_std
        
        self.logger.info(f"Calibrated: mean={self.in_dist_energy_mean:.3f}, std={self.in_dist_energy_std:.3f}")
        self.logger.info(f"Thresholds: warn={self.config.energy_threshold_warn:.3f}, critical={self.config.energy_threshold_critical:.3f}")
