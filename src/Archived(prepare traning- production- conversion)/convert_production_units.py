...
def convert_production_data():
    ...
    for col in accel_cols:
        print(f"  {col}: mean={df[col].mean():10.3f}, std={df[col].std():10.3f}, "
    ...
    for col in accel_cols:
        ...
        print(f"  {col}: mean={new_mean:10.3f} (was {old_mean:10.3f}), "
              f"std={df_converted[col].std():10.3f}, "
              f"min={df_converted[col].min():10.3f}, max={df_converted[col].max():10.3f}")
    print("\nGYROSCOPE (unchanged):")
    for col in gyro_cols:
        print(f"  {col}: mean={df_converted[col].mean():10.3f}, std={df_converted[col].std():10.3f}, "
              f"min={df_converted[col].min():10.3f}, max={df_converted[col].max():10.3f}")
