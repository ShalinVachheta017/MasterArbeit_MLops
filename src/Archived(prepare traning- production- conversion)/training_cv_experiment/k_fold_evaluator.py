...
from config import DATA_PREPARED, OUTPUTS_DIR, LOGS_DIR
...
class KFoldEvaluator:
    ...
    def evaluate_fold(self, model, X_test: np.ndarray, y_test: np.ndarray, 
                     fold_idx: int) -> Dict:
        """
    ...
    def aggregate_results(self) -> Dict:
        ...
        logger.info(f"Precision: {results['precision']['mean']:.3f} ± {results['precision']['std']:.3f}")
        ...
        logger.info(f"\n📋 Per-Fold Breakdown:")
        logger.info(f"{'=' * 60}")
        logger.info(f"{'Fold':<6} {'Accuracy':<10} {'Precision':<10} {'Recall':<10} {'F1-Score':<10}")
        logger.info(f"{'-' * 60}")
        for i, r in enumerate(self.fold_results):
            logger.info(f"{i+1:<6} {r['accuracy']:<10.4f} {r['precision']:<10.4f} "
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"Final Result: Accuracy = {results['accuracy']['mean']:.3f} ± {results['accuracy']['std']:.3f}")
    
    # Compare with paper
    logger.info("")
    logger.info("📚 Comparison with ICTH_16 Paper:")
    logger.info("   Paper (after fine-tuning): 0.870 ± 0.012")
    logger.info(f"   Your result: {results['accuracy']['mean']:.3f} ± {results['accuracy']['std']:.3f}")
