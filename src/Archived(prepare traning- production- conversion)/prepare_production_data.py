...
from config import DATA_PREPARED, DATA_PROCESSED, OVERLAP, PROJECT_ROOT, SENSOR_COLUMNS, WINDOW_SIZE
    ...
    for i in range(0, len(X_scaled) - WINDOW_SIZE + 1, stride):
        ...
        if len(window) == WINDOW_SIZE:
            ...
            window_meta = {
                ...
                "timestamp_start": (
                    str(df.iloc[i]["timestamp_iso"]) if "timestamp_iso" in df.columns else None
                ),
                "timestamp_end": (
                    str(df.iloc[i + WINDOW_SIZE - 1]["timestamp_iso"])
