...
class PipelineDiagnostic:
    ...
    def __init__(
        self, training_config_path: Path, production_metadata_path: Path, production_data_path: Path
    ):
    ...
    def check_sampling_rate(self) -> bool:
        ...
        if prod_window_size != expected_samples:
            ...
            logger.warning(
                f"  ⚠️  Window size doesn't match expected {expected_samples} for 4s @ {train_hz}Hz"
            )
            return False
    ...
    def check_sensor_channels(self) -> bool:
        ...
        if self.production_data.shape[-1] != len(train_cols):
            ...
            logger.error(
                f"  ❌ MISMATCH: Expected {len(train_cols)} channels, got {self.production_data.shape[-1]}"
            )
            return False
    ...
    def _print_summary(self, results: Dict[str, bool]) -> None:
        ...
        if not self.mismatches:
            ...
            logger.info(
                "   → Recommendation: Proceed with controlled fine-tuning on labeled production data"
            )
        else:
