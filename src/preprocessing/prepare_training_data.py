...
from src.config import DATA_PROCESSED, DATA_PREPARED, LOGS_DIR, SENSOR_COLUMNS, ACTIVITY_LABELS
    ...
    def create_sliding_windows(self, df, sensor_cols, data_format):
        ...
        for i in range(n_windows):
            ...
            meta = {
                ...
                'start_idx': start_idx,
                'end_idx': end_idx,
                'timestamp_start': df.iloc[start_idx]['timestamp' if 'timestamp' in df.columns else 'timestamp_ms'],
                'timestamp_end': df.iloc[end_idx-1]['timestamp' if 'timestamp' in df.columns else 'timestamp_ms']
            }
