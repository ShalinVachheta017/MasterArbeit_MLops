...
class TriggerPolicyEngine:
    ...
    def _apply_cooldowns(self, decision: TriggerDecision) -> TriggerDecision:
        ...
        if last_retrain:
            ...
            if datetime.now() - last_retrain_dt < cooldown_period:
                ...
                self.logger.warning(
                    f"Trigger suppressed: cooldown active. "
                ...
                decision.recommendations.insert(
                    0, 
                    f"Cooldown active - retrain queued (wait {time_remaining.seconds//3600}h)"
                )
    ...
    def _update_state(self, decision: TriggerDecision):
        ...
        if self.state['consecutive_warnings'] >= self.thresholds.consecutive_warnings_for_trigger:
            ...
            decision.reason = f"Triggered by {self.state['consecutive_warnings']} consecutive warnings"
...
class ProxyModelValidator:
    ...
    def compare_models(
        ...
        has_regression = (
            comparisons['confidence']['rel_change'] < -self.improvement_threshold or
            comparisons['entropy']['rel_change'] < -self.improvement_threshold
        )
...
def main():
    ...
    parser.add_argument('--reset', action='store_true',
                       help='Reset trigger state')
    parser.add_argument('--summary', action='store_true',
                       help='Show trigger state summary')
