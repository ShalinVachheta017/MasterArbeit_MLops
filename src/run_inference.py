...
from config import (
    DATA_PREPARED, PRETRAINED_MODEL, LOGS_DIR,
    PROJECT_ROOT
)
...
class ModelLoader:
    ...
    def load(self):
        ...
        if input_shape != expected_input:
            self.logger.warning(f"⚠️ Unexpected input shape: {input_shape}, expected {expected_input}")
        
        if output_shape != expected_output:
            self.logger.warning(f"⚠️ Unexpected output shape: {output_shape}, expected {expected_output}")
...
class ResultsExporter:
    ...
    def export(
        ...
        for activity, count in sorted(
            metadata['results_summary']['activity_distribution'].items(),
            key=lambda x: -x[1]
        ):
...
Examples:
  ...
  python run_inference.py --mode realtime
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to preprocessed .npy file"
    )
